...
# Increments: function computing the increments X at steps k, i.e. X_k - X_{k - 1}
Increments = function(X, # the vector of hidden states of the epidemic model (i.e. compartments)
                      x0 # starting points 
)
{
  H    = c()
...
# Kalman: function implementing the Kalman filter (cf Section 3.1, paragraph Incidence data)
...
)
{
  ...
  Delta_X_pred = list() # predicted mean state estimation
  ...
  x_increment =  t(matrix(c(Increments(X = x[- 1, 2], x0 = x[1, 2]),
                            Increments(X = x[- 1, 3], x0 = x[1, 3])), nrow = length(seq(dt, (n + 1) * dt, dt)),
                          ncol = nb_states))
  ...
  Delta_X_pred[[1]] = matrix(c(x_increment[1, 1], x_increment[2, 1]), nrow = nb_states, ncol = 1) 
  Sigma_pred[[1]]   = diag(1 / N, nb_states) 
  
  Delta_X_bar = matrix(c(0, 0), nrow = nb_states, ncol = 1)  
  T_bar       = matrix(c(0, 0, 0, 0), nrow = nb_states, ncol = nb_states)
  ...
  for (k in 1 : n)
  {
    ...
    G_km1 = x_k  - A_km1 %*% x_km1 + (A_km1 - diag(c(1, 1))) %*% matrix(x0, nrow = nb_states, ncol = 1)
    T_km1 = 1 / N * dt * Sigma_km1 
    
    # Filter equations
    
    Delta_X_bar =  Delta_X_bar + Delta_X_pred[[k]] + Sigma_pred[[k]] %*% t(B(p)) %*%
      solve(abs(B(p) %*% Sigma_pred[[k]] %*% t(B(p))) + 1 / N * Vobs(x_increment[, k], p)) %*% 
      (Y[k] - B(p) %*% Delta_X_pred[[k]])
    T_bar       = T_bar + Sigma_pred[[k]] - Sigma_pred[[k]] %*% t(B(p)) %*%
      solve(abs(B(p) %*% Sigma_pred[[k]] %*% t(B(p))) + 1 / N * Vobs(x_increment[, k], p)) %*% 
      B(p) %*% Sigma_pred[[k]]
    
    Delta_X_pred[[k+1]] = G_km1 + (A_km1 - diag(c(1, 1))) %*% Delta_X_bar
    Sigma_pred[[k+1]]   = (A_km1 - diag(c(1, 1))) %*% T_bar %*% t((A_km1 - diag(c(1, 1)))) + T_km1
    
    
  }
  
  output = list(Delta_X_pred = Delta_X_pred, Sigma_pred = Sigma_pred, x_increment = x_increment)
...
)
{
  ...
  Delta_X_pred = res$Delta_X_pred
  Sigma_pred   = res$Sigma_pred
  x_increment  = res$x_increment
  ...
  for(k in 1 : n) 
  {
    s2 = abs(B(p) %*% Sigma_pred[[k]] %*% t(B(p))) + abs(1 / N * Vobs(x_increment[, k], p)) # variance of Y_k given observations y_{k-1},...,y_0
    m  = B(p) %*% Delta_X_pred[[k]]                                               # mean of Y_k given observations y_{k-1},...,y_0 
