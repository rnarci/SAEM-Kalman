...
dt = 0.5 # time interval between observations
...
for(u in 1 : U){
  ...
  for (k in 1 : length(data)){
    ...
    I[k]    = out$data[ind, 3]                           # Markov jump process of the infectious individuals
    data[k] = rbinom(1, I[k], 1 / (1 + exp(- psi2[u])))  # observations
  }
...
theta.init = c(log(1.2), 0.2, 0.4, 0.7) # starting points for the model parameters c(beta_1, beta_2, sqrt(Gamma_1), sqrt(Gamma_2))
...
M_max        = 500             # maximal number of iterations of the SAEM algorithm
M_0          = 300             # number of burn-in iterations of the SAEM algorithm
...
par(mfrow = c(2, 2))
plot(x = 1 : res_SAEM$iter_max, y = res_SAEM$theta[1 : res_SAEM$iter_max, 1], col = "blue", type = "l", xlab = "Iteration", ylab = expression(beta[1]), ylim = c(- 0.2, 0.3))
abline(a = mean(psi1), b = 0, col = "red", lty = 2)

plot(x = 1 : res_SAEM$iter_max, y = res_SAEM$theta[1 : res_SAEM$iter_max, 2], col = "blue", type = "l", xlab = "Iteration", ylab = expression(beta[2]), ylim = c(- 0.2, 0.4))
abline(a = mean(psi2), b = 0, col = "red", lty = 2)

plot(x = 1 : res_SAEM$iter_max, y = res_SAEM$theta[1 : res_SAEM$iter_max, 3], col = "blue", type = "l", xlab = "Iteration", ylab = expression(sqrt(Gamma[1])), ylim = c(0, 0.65))
abline(a = sd(psi1), b = 0, col = "red", lty = 2)

plot(x = 1 : res_SAEM$iter_max, y = res_SAEM$theta[1 : res_SAEM$iter_max, 4], col = "blue", type = "l", xlab = "Iteration", ylab = expression(sqrt(Gamma[2])), ylim = c(0.4, 0.75))
